...
def plot_benchmark(benchmark):
    ...
    if benchmark.dimension != 2:
        ...
        xr = [benchmark.domain.lower_bounds[0], benchmark.domain.upper_bounds[0]]
        yr = [benchmark.domain.lower_bounds[1], benchmark.domain.upper_bounds[1]]
...
def plot_vector_field(
    ...
    XX, YY = np.meshgrid(xx, yy)
    df = f(np.vstack([XX.ravel(), YY.ravel()]).T)
    dx = df[:, 0].reshape(XX.shape)
    dy = df[:, 1].reshape(YY.shape)
...
def plot_results_file(file: str):
    ...
    data = read_csv(file)
    ax = data.plot("Width", "Error", marker="x", linestyle="none")
    plt.show()
