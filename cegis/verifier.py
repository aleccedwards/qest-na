...
from utils import Timer, contains_object, timer, vprint

T = Timer()


def get_verifier(verifier_type: VerifierType):
    if verifier_type == VerifierType.Z3:
        return Z3Verifier
    elif verifier_type == VerifierType.DREAL:
        return DRealVerifier
    else:
        raise ValueError("Invalid verifier type {}".format(verifier_type))
...
class Verifier:
    ...
    def verify(self, f, NN, epsilon=None):
        ...
        for iii, condition in enumerate(tqdm(fmls, disable=not self.verbose)):
            s = self.new_solver()
    ...
    def get_dimensional_constraints(self, f, N, epsilon=[]):
        ...
        if len(error) != len(f):
            raise ValueError(
                "Incorrect number of error bounds; must match dimension n."
            )
        constrs = []
        domain = self.domain(self.vars, _And)
        for f_i, N_i, e_i in zip(f, N, error):
            error_norm = _abs((f_i - N_i).item()) >= e_i
    ...
    def get_absval_constraint(self, f, N, epsilon=None):
        ...
        _abs = self.fncts["abs"]
        error = epsilon if epsilon else self.error
        error_norm = _abs((f - N).item()) >= error
        domain = self.domain(self.vars, _And)
...
class Z3Verifier(Verifier):
    ...
    def recu(x):
        ...
        _If = z3.If
        for idx in range(len(y)):
            y[idx, 0] = z3.simplify(_If(y[idx, 0] > 0, 1, 0))
        return y
    ...
    def _model_result(self, solver, model, x, i):
        try:
            ...
            try:
                ...
                AttributeError
            ):  # when z3 finds non-rational numbers, prints them w/ '?' at the end --> approx 10 decimals
...
class DRealVerifier(Verifier):
    ...
    @staticmethod
    def recu(x):
        """Step function for symbolic DReal variables."""
        y = x.copy()
        _if = dreal.if_then_else
        for idx in range(len(y)):
            y[idx, 0] = _if(y[idx, 0] >= 0, 1, 0)
        return y

    @staticmethod
    def sigmoid(x):
        """Sigmoid function for symbolic DReal variables."""
        y = x.copy()
        _exp = dreal.exp
        for idx in range(len(y)):
            y[idx, 0] = 1 / (1 + _exp(-y[idx, 0]))
        return y

    @staticmethod
    def tanh(x):
        """Tanh function for symbolic DReal variables."""
        y = x.copy()
        _exp = dreal.exp
        for idx in range(len(y)):
            y[idx, 0] = (_exp(2 * y[idx, 0]) - 1) / (1 + _exp(2 * y[idx, 0]))
        return y
    ...
    def _solver_solve(self, solver, fml):
        ...
        if self.is_sat(res) and not self.within_bounds(res):
            ...
            # Note if domain is larger than new bound, this is will spurious results.
            new_bound = 1e2
            fml = dreal.And(
                fml, *(dreal.And(x < new_bound, x > -new_bound) for x in self.vars)
            )
            res = dreal.CheckSatisfiability(fml, 0.0001)
        return res
