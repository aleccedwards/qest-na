...
def worker(cegis_config, id, result, run):
    torch.manual_seed(167 + id)
    attempt = 0
    while run.is_set():
        attempt += 1
        ...
        if res and run.is_set():
            ...
            result["learner" + str(id)] = c.learner
    return result
...
def worker_Q(cegis_config, id, queue, run):
    attempt = 0
    result = None
    torch.manual_seed(167 + id)
    while run.is_set():
    ...
    def run(self, cegis_config):
        ...
        while not stop:
            while len(procs) < self.max_processes and not stop:
                # change config
                p = multiprocessing.Process(
                    target=worker_Q, args=(cegis_config, id, queue, run)
                )
                p.daemon = True
                p.start()
                id += 1
