...
class Polyhedron:
    ...
    def __init__(self, A, L=None) -> None:
        ...
        # Now with floats
        M = cdd.Matrix(A, number_type="float")
        if L is not None:
            M.extend(L, linear=True)
        M.rep_type = cdd.RepType.INEQUALITY
        # M.canonicalize()
        # P = cdd.Polyhedron(M)
        # self.H_array = np.array(tuple(P.get_inequalities()))
        # self.V_array = np.array(tuple(P.get_generators()))
        self.H_array = v_float(self.H)
        self.V_array = v_float(self.V)
        # assert np.allclose(self.H_array, H_array)
        # assert np.allclose(self.V_array, V_array)
    ...
    def reduce_to_nd(self, ndim) -> None:
        ...
        self.H = tuple(P.get_inequalities())
        self.V = tuple(P.get_generators())
        self.H_array = np.array(self.H, dtype=float)
        self.V_array = np.array(self.V, dtype=float)

    def contains(self, x) -> np.ndarray:
        """Check if x lies in polyhedron"""
        x = np.array(x)
        b = self.H_array[:, 0].reshape(-1, 1)
        A = -self.H_array[:, 1:]
        return np.all((x @ A.T).T <= b, axis=0)
    ...
    def border_as_smt(self, x, And):
        """Express border in SMT"""
        x = np.array(x).reshape(-1, 1)
        b = self.H_array[:, 0]
        A = -self.H_array[:, 1:]
        Ax = (A @ x).squeeze()
    ...
    def box_sample(self, N: int):
        ...
        D = [d for d in D if self.contains(d.reshape(1, -1)).item()]
        return D
